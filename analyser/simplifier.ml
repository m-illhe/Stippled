...
let rec simplify_expression expr = 
  match expr with 
  | Unary_operateur(op,e,anno) -> let e = expression_simplifier(e) in (
    match op with
  ...
  | Binary_operator(op, e1, e2, anno) -> let e1, e2 = expression_simplifier(e1), expression_simplifier(e2) in (
    match e1, e2 with
    ...
    | Pos(x1,y1,_), Pos(x2,y2,_) -> (
      match op with 
      | Add | Sub | Mul | Div | Mod -> let x, y = expression_simplifier(Binary_operator(op, x1, x2, anno)), expression_simplifier(Binary_operator(op, y1, y2, anno)) in Pos(x,y,anno)
      | Eq -> expression_simplifier(Binary_operator(Add, x1, x2, anno)), expression_simplifier(Binary_operator(Add, y1, y2, anno)) in  Pos(x,y,anno)
      | Ne -> expression_simplifier(Binary_operator(Add, x1, x2, anno)), expression_simplifier(Binary_operator(Add, y1, y2, anno)) in Pos(x,y,anno)
      | _ -> Binary_operator(op, e1, e2, anno)
    )
    | Color(r,g,b,_), Color(h,s,v_) -> (
      match op with
      | Add | Sub | Mul | Div | Mod -> let r,g,b = expression_simplifier(Binary_operator(op, r, h, anno)), expression_simplifier(Binary_operator(op, g, s, anno)), expression_simplifier(Binary_operator(op, b, v, anno)) in Color(r,g,b,anno)
      | Eq -> expression_simplifier(Binary_operator(Add, r, h, anno)), expression_simplifier(Binary_operator(Add, g, s, anno)), expression_simplifier(Binary_operator(Add, b, v, anno)) in Color(r,g,b,anno)
      | Ne -> expression_simplifier(Binary_operator(Add, r, h, anno)), expression_simplifier(Binary_operator(Add, g, s, anno)), expression_simplifier(Binary_operator(Add, b, v, anno)) in Color(r,g,b,anno)
      | _ -> Binary_operator(op, e1, e2, anno)
    )
    | Point(p1, c1, _), Point(p2, c2, _) -> (
      match op with
      | Add | Sub | Mul | Div | Mod ->  expression_simplifier(Binary_operator(op, p1, p2, anno)), expression_simplifier(Binary_operator(op, 1, 2, anno)) in Point(pos, col, anno)
      | Eq ->  expression_simplifier(Binary_operator(Add, p1, p2, anno)), expression_simplifier(Binary_operator(Add, c1, c2, anno)) in Point(Add, col, anno)
      | Ne ->  expression_simplifier(Binary_operator(Add, p1, p2, anno)), expression_simplifier(Binary_operator(Add, 1, 2, anno)) in Point(pos, col, anno)
      | _ -> Binary_operator(op, e1, e2, anno)
    )
    | _ -> Binary_operator(op, e1, e2, anno)
  )
...
let statement_simplifier statement = statement
let simplifer program = 
  match program with
